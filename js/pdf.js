...
`;
  ...
  if (areas.length) {
    ...
    html += `<table style="width:100%;border-collapse:collapse;margin-bottom:8pt">
      <tr>
        <th ${TH}>Área</th>
        <th ${TH}>Dimensiones</th>
        <th ${TH}>Superficie</th>
        <th ${TH}>Orientación</th>
        <th ${TH}>Inclinación</th>
        <th ${TH}>Pisos</th>
        <th ${TH}>Tablero→Inv.</th>
        <th ${TH}>Inv.→Paneles</th>
      </tr>`;
    for (const a of areas) {
      const dim = (a.ancho && a.largo) ? `${a.ancho} × ${a.largo} m` : '—';
      const sup = (a.ancho && a.largo) ? `${(a.ancho * a.largo).toFixed(1)} m²` : (a.area ? `${a.area} m²` : '—');
      html += `<tr>
        <td ${TD}>${esc(a.nombre || '—')}</td>
        <td ${TD}>${dim}</td>
        <td ${TD}>${sup}</td>
  ...
  if (tieneElec && (lev.tipoServicioCFE || lev.tierraFisica || lev.centroCarga || lev.tarifaCFE)) {
    ...
    if (aparatos.length) {
      ...
      for (const ap of aparatos) {
        ...
        html += `<tr>
          <td ${TD}>${esc(ap.nombre || '—')}</td>
          <td ${TD}>${ap.cantidad || 1}</td>
          <td ${TD}>${ap.watts != null ? ap.watts : '—'}</td>
...
<body>${html}</body>
  ...
  const a    = document.createElement('a');
  a.href     = url;
  a.download = `EFS-Levantamiento-${project.displayId}.doc`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  setTimeout(() => URL.revokeObjectURL(url), 2000);
